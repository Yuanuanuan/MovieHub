...
```
src/
├── api/            # Axios 實例與 TMDB API 封裝（movie.ts / person.ts）
...
| 變數 | 說明 |
| --- | --- |
| `VITE_API_TOKEN` | TMDB API Read Access Token（必填，需自行至 TMDB 帳號設定申請） |
| `VITE_BASE_URL` | TMDB API 基底網址，預設 `https://api.themoviedb.org/3` |
